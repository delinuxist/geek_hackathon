              ...
              ourPeoples.map((p, index) => (
                ...
                >
                  <div className={`lg:h-[${p.image.height}] w-[${p.image.width}] border-2 rounded-lg duration-200 hover:scale-105 shadow-2xl relative group ease-in-out`}>
                    ...
                    <Image src={p.image.url} alt={p.info.name} height={p.image.height} width={p.image.width} className={' object-cover rounded-lg rounded-b-none lg:rounded-lg'} />
                    <CustomButton containerStyles='py-2 bg-inspiration hidden lg:group-hover:block absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 hover:scale-105 duration-200 ease-in-out' title="View Bio" textStyles=' font-bold text-software-grey hover:text-white' handleClick={() => openModal(p.info)} />
